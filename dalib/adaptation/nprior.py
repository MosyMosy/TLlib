...
class NearestPrior(nn.Module):
    ...
    @staticmethod
    def reg_loss_entropy(Feature_all, source_size, sigma=1):
        ...
        similarity_all = torch.exp(-dist_all / (
            2 * (var_all * sigma) ** 2)) * (1 - torch.eye(all_size))
        ...
        source_probs = torch.stack([source_prob, 1 - source_prob], dim=1)
        target_probs = torch.stack([target_prob, 1 - target_prob], dim=1)

        max_entropy = torch.log2(torch.tensor(2)) * 2
        return max_entropy - (NearestPriorentropy(source_probs) + NearestPriorentropy(target_probs))

    @staticmethod
    def reg_loss_dist(Feature_all, source_size, sigma=1):
        all_size = Feature_all.size()[0]
        dist_all = torch.cdist(Feature_all, Feature_all, p=2,
                               compute_mode='use_mm_for_euclid_dist_if_necessary')
