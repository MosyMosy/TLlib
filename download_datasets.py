...
for dataset in datasets.__all__:
    dataset =  datasets.__dict__[dataset]("", download=False)
    list(map(lambda args: download(os.path.join("~/scratch/TLlib_Dataset", dataset.lower()), *args),dataset.download_list))
